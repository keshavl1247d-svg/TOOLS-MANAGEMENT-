  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-semibold text-gray-dark">Part Management</h2>
        {isAdmin && (
          <button 
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-gray-divider">
              {loading ? (
                <tr><td colSpan="7" className="px-6 py-8 text-center text-gray-text">Loading parts...</td></tr>
              ) : parts.length === 0 ? (
                <tr><td colSpan="7" className="px-6 py-8 text-center text-gray-light italic">No parts found.</td></tr>
              ) : (
                parts.map(part => (
                  <tr key={part.id} className="hover:bg-gray-50/50 transition-colors">
                    <td className="px-6 py-3.5 text-gray-dark font-medium">{part.part_number}</td>
                    <td className="px-6 py-3.5 text-gray-text">{part.part_name}</td>
                    <td className="px-6 py-3.5 text-gray-text">{part.quantity}</td>
                    <td className="px-6 py-3.5 text-gray-text">{part.location || '—'}</td>
                    ...
                    <td className="px-6 py-3.5 flex justify-center">
                      {part.photo ? (
                        <img 
                          ...
                          onError={(e) => { e.target.onerror = null; e.target.src = 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="40" height="40"><rect width="40" height="40" fill="%23f1f3f4"/><text x="50%" y="50%" font-family="sans-serif" font-size="10" fill="%239aa0a6" text-anchor="middle" dy=".3em">No img</text></svg>'; }}
                        />
                      ) : (
      ...
      <Modal isOpen={isModalOpen} onClose={handleCloseModal} title={editingPart ? 'Edit Part' : 'Add Part'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-semibold text-gray-text uppercase tracking-wide mb-1.5">Part Number *</label>
              <input type="text" value={formData.part_number} onChange={e => setFormData({...formData, part_number: e.target.value})} className="w-full px-3 py-2 border border-gray-border rounded focus:border-primary focus:ring-1 focus:ring-primary outline-none" required />
            </div>
            <div>
              <label className="block text-xs font-semibold text-gray-text uppercase tracking-wide mb-1.5">Quantity</label>
              <input type="number" value={formData.quantity} onChange={e => setFormData({...formData, quantity: e.target.value})} className="w-full px-3 py-2 border border-gray-border rounded focus:border-primary focus:ring-1 focus:ring-primary outline-none" min="0" />
            </div>
          </div>
          
          <div>
            <label className="block text-xs font-semibold text-gray-text uppercase tracking-wide mb-1.5">Part Name *</label>
            <input type="text" value={formData.part_name} onChange={e => setFormData({...formData, part_name: e.target.value})} className="w-full px-3 py-2 border border-gray-border rounded focus:border-primary focus:ring-1 focus:ring-primary outline-none" required />
          </div>

          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-semibold text-gray-text uppercase tracking-wide mb-1.5">Location</label>
              <input type="text" value={formData.location} onChange={e => setFormData({...formData, location: e.target.value})} className="w-full px-3 py-2 border border-gray-border rounded focus:border-primary focus:ring-1 focus:ring-primary outline-none" />
            </div>
            <div>
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-text uppercase tracking-wide mb-1.5">Photo</label>
            <div className="mt-1 flex items-center gap-4">
              {previewUrl ? (
                ...
                <div className="w-16 h-16 rounded border border-gray-divider bg-gray-50 flex items-center justify-center text-gray-light text-xs">No img</div>
              )}
              <input 
                type="file" 
            ...
            <p className="mt-1 text-xs text-gray-light">Max size: 5MB. Formats: JPG, PNG, WEBP.</p>
          </div>

          <div className="flex justify-end gap-3 pt-4 border-t border-gray-divider mt-6">
            <button type="button" onClick={handleCloseModal} className="px-4 py-2 text-sm font-medium text-gray-text bg-gray-50 hover:bg-gray-200 rounded transition-colors">Cancel</button>
